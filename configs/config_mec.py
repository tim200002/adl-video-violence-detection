...
dim = 600
warmpup_epochs = 0
# mec_weighting_factor = 2.0
mec_weighting_factor = 1.0
...
transform = transforms.Compose([
    T.ToFloatTensorInZeroOne(),
    T.RandomResizedCrop((172, 172), scale=(0.2, 1)),
    transforms.RandomApply([T.VideoColorJitter(0.4, 0.4, 0.2, 0.1)], p=0.8),
    transforms.RandomApply([T.GrayScale()], p=0.2),
    transforms.RandomApply([T.GaussianBlur((5, 9))], p=1.0),
    T.RandomHorizontalFlip(),
])
...
transform2 = transforms.Compose([
    ...
    transforms.RandomApply([T.VideoColorJitter(0.4, 0.4, 0.2, 0.1)], p=0.8),
    transforms.RandomApply([T.GrayScale()], p=0.2),
    transforms.RandomApply([T.GaussianBlur((5, 9))], p=0.1),
    transforms.RandomApply([T.Solarize(0.5)], p=0.2),
    T.RandomHorizontalFlip(),
])
...
train_dataset_hockey = dataloader.Hockey(root=train_path_hockey,  frames_per_clip=num_frames,step_between_clips = clip_steps,  train=0,transform=transform_src, num_workers=16)
...
train_dataset_ucf = dataloader.UCF(root=train_path_ucf, frames_per_clip=num_frames,frame_rate=frame_rate,step_between_clips = clip_steps,  train=0,transform=None, num_workers=8)
train_dataset_ucf = dataloader.DoubleTransformDataset(train_dataset_ucf, transform, transform2)
