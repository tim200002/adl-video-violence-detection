...
def train(config):
    ...
    for epoch in range(1, no_of_epoch + 1):
        ...
        for _ in tqdm(range(0, iterations_this_epoch)):
            ...
            # forward pass
            preds, src_features = model.encoder(src_videos.cuda())
            z1, z2, p1, p2, f1, f2 = model(target_videos1.cuda(), target_videos2.cuda())
            target_features = (f1 + f2) / 2 # average the two augmented features
            ...
            loss = loss_acc - config.mec_weighting_factor * lamda_inv * ssl_loss + config.mmd_weighting_factor * loss_alignment
